...
public final class RightHandSlotFeature extends SlotFeature {

    @Override
    protected ItemStack getSlot(GUIMetaData meta) {
        return Objects.requireNonNull(meta.getArmorStand().getEquipment()).getItemInMainHand();
    }

    @Override
    protected void setSlot(GUIMetaData meta, ItemStack is) {
        Objects.requireNonNull(meta.getArmorStand().getEquipment()).setItemInMainHand(is);
    }

    @Override
    protected boolean check(Player player, GUIMetaData meta, ItemStack oldItem, ItemStack newItem) {
        var event = new PlayerArmorStandManipulateEvent(player, meta.getArmorStand(), newItem, oldItem, EquipmentSlot.HAND);
