...
public final class BaseFeature implements Feature {
    ...
    public ItemStack getItem(Player player, GUIMetaData meta) {
        var is = new ItemStack(Material.SMOOTH_STONE_SLAB);
        var im = Objects.requireNonNull(is.getItemMeta());
        im.setDisplayName(ChatColor.YELLOW + "Show Base Plate");
        im.setLore(List.of(ChatColor.AQUA + "Status: " + Util.genStatus(meta.getArmorStand().hasBasePlate())));
