...
public final class BodyPoseFeature implements Feature {

    @Override
    public ItemStack getItem(Player player, GUIMetaData meta) {
        var is = new ItemStack(Material.GOLDEN_CHESTPLATE);
        var im = Objects.requireNonNull(is.getItemMeta());
        im.setDisplayName(ChatColor.YELLOW + "Edit Body Pose");
    ...
    public void operate(Player player, GUIMetaData meta, ClickType click, ItemStack oldItem, ItemStack newItem) {
        Bukkit.getScheduler().runTask(Util.getPlugin(), () -> {
            ...
            EditorManager.INSTANCE.setEditor(player, new EulerAngleEditor(meta.getArmorStand(), EulerAngleEditor.PoseType.BODY));
        });
    }
